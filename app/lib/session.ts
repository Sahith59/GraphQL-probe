...
const COOKIE_NAME = "bold_graphql_session";
const SESSION_SECRET = process.env.SESSION_SECRET || "bold-graphql-local-secret";
...
export async function requireUserResponse() {
  ...
  if (!user) {
    return {
      user: null,
      response: NextResponse.json({ errors: [{ message: "Authentication required" }] }, { status: 401 })
    };
  }
